...
function enhanceAdvertisement(){const ad=document.querySelector(".ad");if(plusActive()){ad?.remove();return}if(!ad||ad.querySelector(".ad-request"))return;const link=document.createElement("a");link.className="ad-request";link.textContent="درخواست ثبت تبلیغ";link.href=emailLink("درخواست تبلیغات در روزنگار","سلام، برای ثبت تبلیغ در روزنگار درخواست دارم.");link.target="_blank";link.rel="noopener";ad.querySelector("div")?.appendChild(link)}
...
function injectAdvanced(){
 ...
 section.innerHTML='<div class="box '+(active?'unlocked':'locked')+'"><small>امکانات پلاس</small><h2>🔔 یادآور '+(active?'فعال':'قفل است')+'</h2>'+(active?'<input id="adv-title" placeholder="عنوان یادآور"><input id="adv-time" type="datetime-local"><button id="adv-remind">تنظیم یادآور</button><p><small>برای دریافت اعلان، صفحه را باز نگه دارید.</small></p>':'<p>یادآورها و امکانات پیشرفته برای خریداران نسخه پلاس فعال می‌شود.</p><button id="activate-plus">واردکردن کد فعال‌سازی</button>')+'</div><div class="box plus"><small>روزنگار پلاس</small><h2>نسخه پیشرفته‌تر</h2><p>یادآور، حذف تبلیغات و امکانات پیشرفته آینده.</p>'+(active?'<b class="active-badge">✓ نسخه پلاس فعال است</b>':'<a target="_blank" rel="noopener" href="'+emailLink('درخواست خرید روزنگار پلاس','سلام، برای خرید نسخه پلاس و دریافت کد فعال‌سازی درخواست دارم.')+'">درخواست خرید و دریافت کد</a>')+'</div><div class="box"><h2>ارتباط با ما</h2><p>برای پشتیبانی، پیشنهادها و گزارش مشکل پیام بفرستید. برای دریافت کد فعال‌سازی ایمیل دهید.</p><a target="_blank" rel="noopener" href="'+emailLink('پیام از روزنگار','سلام، پیام من درباره روزنگار:')+'">ارسال ایمیل</a><small class="email">'+CONTACT_EMAIL+'</small></div><div class="box wide"><h2>♡ حمایت مالی</h2><p>اطلاعات پرداخت عمومی هنوز ثبت نشده است. برای هماهنگی حمایت پیام بفرستید.</p><a target="_blank" rel="noopener" href="'+emailLink('حمایت از روزنگار','سلام، می‌خواهم از توسعه روزنگار حمایت کنم. لطفاً روش پرداخت را اعلام کنید.')+'">هماهنگی برای حمایت</a></div>';
